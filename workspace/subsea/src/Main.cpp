...
int main(int argc, char** argv)
{
	char c = 'y';
	CSubSeaAlgo Algo;
	string StrLargeFileName,StrSmallFileName,line;
	pair<int,int> RetVal;
	clock_t T1;
        string our_input_param;

        if (argc<3)
          {
             cout << "FORMAT: subsea <larget file list> <small file list>"<<endl;
             return -1;
          }
        if (argc>=4)
          {
            our_input_param = argv[3];
            if (our_input_param.compare("gtr")==0)
            {
                ourFormat = false;
                // check for label data
                if (argc>=5)
                    NumberOfLabels = atoi(argv[4]);
                if (NumberOfLabels<=0)
                    NumberOfLabels = 0;
            }
          }
	
	 ifstream myfileLarge (argv[1]);
	 ifstream myfileSmall (argv[2]);
	  
	 ofstream outL("outfileL.txt");
	 ofstream outS("outfileS.txt");
	 if (myfileLarge.is_open() && myfileSmall.is_open())
	  {
	   while ( ! myfileLarge.eof() && !myfileSmall.eof())
		{
		// cout<<"-----------Compare---123------------------------------"<<endl; 
		  ...
		  //calcAlg(lineLarge,lineSmall);
		 	...
		 	if (Algo.Init(StrSmallFileName, StrLargeFileName))
			{
                ...
                //cout<<"Entering Algo.Run..."<<endl;
				RetVal = Algo.Run();
			
			
				cout << StrLargeFileName << " " << StrSmallFileName ;
				cout << " " << RetVal.first <<" "<<(double(clock() - T1) / CLOCKS_PER_SEC )<< endl;
			}
