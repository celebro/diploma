...
\TODO{Intro.}
	...
	Prevdokoda agloritma je podana v \refalg{alg:ullmann1}. V vrsticah 1--3 inicializiramo vse spremenljivke, začnemo z začetno matriko $M^0$, 
	na globini 1 in brez izbranega stolpca. V vrstici 3 shranimo matriko $M$ na prvem koraku - matriko vedno shranjujemo pred vstopom v naslednji korak. 	
	...
	algoritem sestopi, če ne najdemo ustreznega stolpca. V vrstici 6 preverimo, če v trenutni vrstici obstaja še neobiskan stolpec (zaenkrat ignoriramo del v 	
	oglatih oklepajih). Ta stolpec mora biti še neizbran v trenutni vrstici ($j > k$), biti združljiv s trenutno vrstico ($m_{d,j} = 1$) in ne sme biti izbran v
	katerem od prejšnjih korakov ($F_j=0$; pogoj za injektivno preslikavo). Sama izbira stolpca poteka v vrsticah 7--9, ko $k$ postavimo na izbrani stolpec.
	V vrstici 10 postavimo vse neizbrane stolpce v vrstici na $0$, torej izbrani stolpec označimo tudi v matriki $M$. Če nismo prišli do dna preiskovalnega
	drevesa (vrstica 11), gremo v naslednji korak (vrstica 12): shranimo zadnji izbrani stolpec na trenutni globini ($H_d \gets k$), stolpec označimo kot
	že izbranega ($F_k \gets 1$), povečamo globino ($d \gets d + 1$), resetiramo izbiro stolpca za naslednji korak ($k \gets 0$), shranimo matriko $M$
	za naslednji korak ($M_d \gets M$; od tukaj bomo obnavljali matriko $M$ ob vračanju v tisti korak) in preprečimo sestopanje ($backtrack \gets false$).
	Če smo obdelali že vse vrstice v matriki $M$, potem v vrstici 14 preverimo, če matrika ustreza pogoju~(\ref{eq:ullmann1}), torej če matrika predstavlja
	podgrafni izomorfizem. V pozitivnem primeru jo ustrezno shranimo ali izpišemo. V vrstici 16 obnovimo matriko $M$ in preprečimo sestopanje. Tako bomo
	v naslednji ponovitvi zanke preverili, če je v zadnji vrstici še kakšen primeren stolpec. Vrstice 17--20 skrbijo za sestopanje. V vrstici 18 sprostimo zadnji
	izbrani stolpec ($F_k \gets 0$) in znižamo globino. Nato v vrstici 20 obnovimo matriko $M$ za ta korak in v $k$ obnovimo nazadnje izbrani stolpec na 
	tej globini.
...
\begin{algorithmic}[1]
	...
	\While{$d \not = 0$}																		
		...
		\If{$(\exists j : j > k \wedge m_{d,j} = 1 \wedge F_j = 0) \;\; [\; \wedge \; refine(M, P, T)]$}			
			%\State $$													
			\Repeat 
				\State $k \gets k +1$ 															
			...
			\If {$d < n_p$}																	
				\State $H_d \gets k; F_k \gets 1; d \gets d+1; k \gets 0; M_d \gets M; backtrack \gets false$		
			\Else																			
				\If {$<$ condition(\ref{eq:ullmann1}) $>$}											
					\State $<$ store $M$ $>$													
				\EndIf													
				%\If{$\exists j : j > k \wedge m_{d,j} = 1 \wedge F_j = 0$}								
					\State $M \gets M_d; backtrack \gets false$										
				%\EndIf
			\EndIf
		\EndIf
...
\TODO{ta pogoj lahko nadomesti pogoj z množenjem matrik. ekvivalenca v primeru induciranega podgrafa}
...
\begin{algorithmic}[1]
	\Require Matriki sosednosti $P$ in $T$, začetna matrika $M^0$
	\Ensure Seznam $result$ $n_p \times n_t$ matrik $M$, ki predstavljajo preslikave podgrafnih izomorfizmov
	\State $M \gets M^0;  result \gets [\;]$
	\For{$i $ in$ [1, n_p]$} $F_i \gets 0;$ \EndFor
	\State $step(1)$
	\Statex
	\Function{step}{d}
