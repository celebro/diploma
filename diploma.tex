...
\TODO{ta pogoj lahko nadomesti pogoj z množenjem matrik}
...
\begin{algorithmic}[1]
	...
	\State $M \gets M^0; d \gets 1; H_1 \gets 0; k \gets 0; backtract \gets true; result \gets [\;]$
	...
	\While{$d \not = 0$}
		...
		\If{$(\exists j : j > k \wedge m_{d,j} = 1 \wedge F_j = 0) \;\; [\; \wedge \; refine(M, P, T)]$}
			%\State $$
			\Repeat \State $k \gets k +1$ \Until{$m_{d,k} = 1 \vee F_k = 1$}
			\State $\forall j \not = k : m_{d,j} \gets 0$
