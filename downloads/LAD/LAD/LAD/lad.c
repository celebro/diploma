...
bool checkLAD(int u, int v, Tdomain* D, Tgraph* Gp, Tgraph* Gt){
	...
	for (i=0; i<Gp->nbSucc[u]; i++){
		...
		if (D->nbVal[u2]>Gt->nbSucc[v]){
			...
			for (j=0; j<Gt->nbSucc[v]; j++){
				...
				if (isInD(u2,v2,D)){ // v2 belongs to D[u2]
					if (num[v2]<0){ // v2 has not yet been added to V
						num[v2]=nbNum;
						numInv[nbNum++]=v2;
					} 
	...
	if (!updateMatching(Gp->nbSucc[u],nbNum,nbComp,firstComp,comp,matchedWithU)) return false;
