...
\chapter{Algoritem VF2}
	...
	$M$, jih ta algoritem gradi postopoma. V vsakem stanju $s$ algoritma imamo delno preslikavo $M(s)$. Ta definira podgrafa $G_p(s) \subseteq G_p$ in
...
\end{algorithm}
	...
	naslednjih množic:
	\begin{enumerate}
	\item $T^+(s) = N^+(M_p(s)) \times N^+(M_t(s))$
	\item $T^-(s) = N^-(M_p(s)) \times N^-(M_t(s))$\;\;\; če je $T^+(s)$ prazna
	\item $P^d(s) = (V_p \setminus (M_p(s) \cup T_p(s)) )  \times (V_t \setminus (M_t(s) \cup T_t(s)) )$\;\;\; če je $T^-(s)$ prazna
	\end{enumerate}
	...
	\label{pic_t_in_out}
	\end{figure}
	\TODO{pic, praksa vyamemo enega iz tp, preverimo vse iz tt, ker mora veljati ya vsakega}
	V sami implementacije algoritma množica $P$ ni eksplicitno generirana. Total order
	...
	\begin{equation}
	\begin{array}{rcl}
	|N_p^-(n) \cap T_p^-(s)|   \! & \! \leq \! & \! |N_t^-(s) \cap T_t^-(s)| \; \wedge \\
	|N_p^+(n) \cap T_p^-(s)|  \! & \! \leq \! & \! |N_t^+(s) \cap T_t^-(s)|
	\end{array}
	\end{equation}
