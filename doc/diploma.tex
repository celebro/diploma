...
\end{algorithm}
	...
	Zaporedno številko obiskovanja označuje preslikava $d : V \to \mathbb{N}$. Za vsako vozlišče definiramo še $l_i = l(v): d(v) = i$ in $N_i = \{ d(u) < i:
	u \in N_G(v) \, \wedge \, d(v) = i\}$.
...
\begin{algorithmic}[1]
	...
	\While {$N_0 \not = \emptyset$}
		\State \textbf{choose} $w \in N_0$ with miminal \Call{EstimateNext}{$w, v$}
		\If {$d(w) = 0$} 
			\Call{Visit}{$w$}
		\EndIf
...
\begin{algorithmic}[1]
	...
	\State $g(v_1') \gets 1$
	\State $g(v_2') \gets 2$
	\State \Return \Call{SearchVisit}{$\{ v_1', v_2' \}, \emptyset$}
	...
	\For {$w \in L$}
		\If {$g(w) = 0$ [ \textbf{and} $(v,w) \not \in ``Black"$ ]}
			...
			\State $S \gets S \cup$ \Call{SearchVisit}{$V' \cup \{w\}, E'$}
			\State $g(w) \gets 0$
		\EndIf
	\EndFor
	\State \Return $S$
