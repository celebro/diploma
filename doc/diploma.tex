...
\end{algorithm}
	...
	V funkcije $searchVisit$ naprej dobimo vrednost zadnje zaporedne številke iz $g$ in usrezno vozlišče (zadnje dodano vozlišče, vrstici \ref{line:st6}
	...
	pridejo zaradi prej dodanega vozlišča. Povezave potrebujemo, da lahko vrnemo celoten izomorfen podgraf, ko smo ga našli (vrstica  \ref{line:st13}).
	...
	$H[vtime + 1].N$ (vrstica \ref{line:st14}). V vrsticah \ref{line:st17}--\ref{line:st20} te kandidate preverimo. 
...
\begin{algorithmic}[1]
	...
	\For {$w \in L$}
		\If {$g(w) = 0$ [ \textbf{and} $(v,w) \not \in ``Black"$ ]}			\label{line:st17}
			\State $g(w) \gets vtime$								\label{line:st18}
...
\end{algorithm}
	...
	Ko smo našli izomorfizem, je $d_t^{-1}\circ d_p$
...
\begin{algorithmic}[1]
	...
	\Ensure Vsi podgrafi [ ali inducirani podgrafi ] grafa $G_t$, ki so izomorfni grafu $G_p$
	 ...
	 \For {$\langle v_1, v_2 \rangle \in V \times V$}
